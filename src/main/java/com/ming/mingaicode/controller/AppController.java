...
public class AppController {
    ...
    public void downloadAppCode(@PathVariable Long appId,
        ...
        if (!app.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "不能下载别人的应用代码");
        }
        //构建应用代码目录路径(生成的路径 而不是部署的路径)
    ...
    public BaseResponse<Boolean> updateApp(@RequestBody AppUpdateRequest appUpdateRequest, HttpServletRequest request) {
        ...
        // 仅本人可更新
        if (!oldApp.getUserId().equals(userService.getLoginUser(request).getId())) {
     ...
     * @return 精选应用列表
     */
    //T(类名)：用于调用静态方法，生成缓存 key #参数名：用于引用方法参数 condition：设置缓存条件，只有前 10 页才会被缓存
    @PostMapping("/good/list/page/vo")
    @Cacheable(
            value = "good_app_page",
            key = "T(com.ming.mingaicode.utils.CacheKeyUtils).generateKey(#appQueryRequest)",
